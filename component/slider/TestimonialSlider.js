...
const TestimonialSlider = ({ items }) => {
  const data = items && items.length > 0 ? items : testimonialData;
      ...
      {data.map((item) => (
        <div className="col-xl-6 wow fadeInUp" key={item.id}>
          <div className="tf__single_testimonial">
            ...
            <p className="description">{item.description || item.desc}</p>
            {(item.imagePath || item.imgSrc) && (
              <div className="img">
                <img src={item.imagePath || item.imgSrc} alt="client" className="img-fluid w-100" />
              </div>
            )}
